...
- After local gates pass, delegate a read-only review to a fresh reviewer agent that did not implement the change.
...
- For a material task, create or reuse a `codex/<task-name>` branch and open a draft PR automatically. Follow-ups update the existing task PR.
...
- New commits invalidate previous verification: rerun affected local gates, required CI, and independent review.
